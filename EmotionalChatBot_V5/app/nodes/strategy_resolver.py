...
修复要点（不改其他功能）：
1) 修复量纲混用：EMA 里 m_prev*100 与 e_turn(0~10) 统一到同一量纲（e_turn*10 -> 0~100）
2) 修复 arousal 归一化：支持输入为 [-1,1]（0 为中性）或 [0,1]（0.5 为中性），由配置/启发式决定
3) 调整 fatigue：默认从 30 轮后才开始惩罚，且每轮惩罚更温和（可被 yaml 覆盖）
4) min_momentum 门控使用 m_next（本轮读完输入后的动量），避免策略迟钝
"""
...
from app.nodes.strategy_routers import (
    BUSYNESS_THRESHOLD as _ROUTER_BUSYNESS_THRESHOLD,
    HIGH_STAKES_IDS,
    EMOTIONAL_GAME_IDS,
    FORM_RHYTHM_IDS,
)
...
# 每轮开始与写回时 conversation_momentum 的下限
MOMENTUM_FLOOR = float(_MOMENTUM_FORMULA_CONFIG.get("momentum_floor", 0.4))
...
# 启动时校验：三路 router 的 id 必须都在 STRATEGY_PRIORITY_13 中，避免配置不同步
def _assert_router_ids_in_priority() -> None:
    router_ids = HIGH_STAKES_IDS | EMOTIONAL_GAME_IDS | FORM_RHYTHM_IDS
    missing = [sid for sid in router_ids if sid not in STRATEGY_PRIORITY_13]
    if missing:
        raise AssertionError(
            f"STRATEGY_PRIORITY_13 与 strategy_routers 不同步：{missing} 未出现在优先级列表中"
        )


_assert_router_ids_in_priority()

# 与 strategy_routers 共用：>= 则 Bot 状态为「忙碌」
BUSYNESS_THRESHOLD = _ROUTER_BUSYNESS_THRESHOLD

# Deferral_Score 超过此阈值时公式触发 busy_deferral
DEFERRAL_SCORE_THRESHOLD = 0.5
...
def _resolve_from_three_routers(
    ...
    if state is not None:
        ...
        if deferral_score > DEFERRAL_SCORE_THRESHOLD:
            candidates.append("busy_deferral")
            logger.info("[StrategyResolver] Deferral_Score=%.3f > %s，公式触发 busy_deferral", deferral_score, DEFERRAL_SCORE_THRESHOLD)
        ...
        if boundary_id and boundary_id in STRATEGY_PRIORITY_13:
            ...
            logger.info("[StrategyResolver] Stage 越界判定命中: %s", boundary_id)
        ...
        for sid in candidates:
            ...
            try:
                ...
                if m_current >= min_m_f:
                    ...
                    logger.info("[StrategyResolver] 剔除 %s：当前动量 %.3f < min_momentum %.3f", sid, m_current, min_m_f)
            except (TypeError, ValueError):
                filtered.append(sid)
        candidates = filtered
...
def _arousal_clamped(x: Any) -> float:
    """
    PAD Arousal 归一化到配置的 range_min..range_max。
    支持两种输入：
    - 直接输入在 [-1,1]（0 为中性）：将其 clamp 到 range_min..range_max
    - 输入在 [0,1]（0.5 为中性）：先映射到 [-1,1] 再 clamp

    选择逻辑（尽量不破坏现有链路）：
    - 若配置里显式指定 input_scale == "01"：按 [0,1] 解释
    - 否则：优先按 [-1,1] 解释（与你日志里 0.0=中性更一致）
    """
    ar = _MOMENTUM_FORMULA_CONFIG.get("arousal") or {}
    r_min = float(ar.get("range_min", -1.0))
    r_max = float(ar.get("range_max", 1.0))
    input_scale = str(ar.get("input_scale", "") or "").strip().lower()  # 可选: "01" / "-11"
    ...
    # 显式声明
    if input_scale in ("01", "0_1", "0-1", "unit"):
        v = max(0.0, min(1.0, v))
        v = (v - 0.5) * 2.0  # 0.5->0, 0->-1, 1->+1
        return max(r_min, min(r_max, v))

    if input_scale in ("-11", "-1_1", "-1..1", "-1-1"):
        return max(r_min, min(r_max, v))

    # 启发式：默认按 [-1,1] 解释（0 为中性），更贴合常见 PAD 与你日志表现
    if -1.0 <= v <= 1.0:
        return max(r_min, min(r_max, v))

    # 兜底：若输入超出 [-1,1]，但落在 [0,1]，按 unit scale 解释
    if 0.0 <= v <= 1.0:
        v = (v - 0.5) * 2.0
        return max(r_min, min(r_max, v))
...
def _compute_momentum_next(state: Dict[str, Any]) -> float:
    ...
    weight_e_user = float(_MOMENTUM_FORMULA_CONFIG.get("e_turn_e_user_weight", 0.3))
    weight_t_bot = float(_MOMENTUM_FORMULA_CONFIG.get("e_turn_t_bot_weight", 0.4))
    weight_r_base = float(_MOMENTUM_FORMULA_CONFIG.get("e_turn_r_base_weight", 0.3))

    e_turn = (e_user * weight_e_user) + (t_bot * weight_t_bot) + (r_base * weight_r_base) - (h_user * hostility_penalty_coef)
    ...
    fatigue_cfg = _MOMENTUM_FORMULA_CONFIG.get("fatigue") or {}
    fatigue_start_turn = int(fatigue_cfg.get("start_turn", 30))
    fatigue_per_turn = float(fatigue_cfg.get("per_turn", 1.0))
    penalty_fatigue = max(0.0, (turn_count - fatigue_start_turn) * fatigue_per_turn)

    ema_alpha = float(_MOMENTUM_FORMULA_CONFIG.get("ema_alpha", 0.3))

    # 关键修复：把 e_turn(0~10) 拉到 0~100 再 EMA
    e_turn_100 = (e_turn * 10.0) * multiplier_arousal

    score_raw = (
        (m_prev * 100.0) * (1.0 - ema_alpha)
        + e_turn_100 * ema_alpha
    ) - penalty_fatigue
    ...
    return max(MOMENTUM_FLOOR, min(1.0, m_next))
...
def create_strategy_resolver_node() -> Callable[[AgentState], dict]:
    ...
    def node(state: AgentState) -> dict:
        ...
        if skip_reply:
            logger.info("[StrategyResolver] 当轮动量 M_next=%.3f < %s，跳过回复 (skip_reply=True)", m_next, SKIP_REPLY_MOMENTUM_THRESHOLD)
            ...
            if fast_quick_reply_enabled:
                ...
                if isinstance(dur_list, list) and len(dur_list) >= 2:
                    ...
                    if len(first_two) >= 2 and sum(first_two) < threshold_sec:
                        ...
                        logger.info("[StrategyResolver] 前两条回复总用时 %.1fs < %ss，force_fast_route=True", sum(first_two), threshold_sec)
        ...
        logger.info("[StrategyResolver] 三路路由 raw: high_stakes=%r, emotional_game=%r, form_rhythm=%r | M_prev=%.3f M_next(公式)=%.3f", hid, eid, fid, m_prev, m_next)

        # 2）按 min_momentum 过滤后的候选里按优先级选一个
        # 修复：用 m_next（读完用户输入后的动量）做 min_momentum 门控，避免策略迟钝
        chosen_id = _resolve_from_three_routers(hid, eid, fid, state_dict, m_current=m_next)
        ...
        if not chosen_id:
            ...
            if not chosen_id:
                logger.info("[StrategyResolver] 13 级均未命中且 momentum 未算出，current_strategy 置空")
                return {
                    "current_strategy_id": None,
                    "current_strategy": None,
                    "conversation_momentum": max(MOMENTUM_FLOOR, min(1.0, m_next)),
                    "skip_reply": skip_reply,
                    "force_fast_route": force_fast_route,
                }
            logger.info("[StrategyResolver] 走常态动量，M_next=%.3f，选用: %s", m_next, chosen_id)
            new_momentum = m_next
        else:
            ...
            if chosen_id in MOMENTUM_IDS:
                ...
                logger.info("[StrategyResolver] 特殊信号 %s，modifier=%s，M: %.3f -> %.3f", chosen_id, mod, m_prev, new_momentum)

        # 写回时兜底：不得低于 momentum_floor
        new_momentum = max(MOMENTUM_FLOOR, min(1.0, new_momentum))

        strategy = get_strategy_by_id(chosen_id)
        if not strategy:
            logger.warning("[StrategyResolver] 未找到策略 id=%s，置空", chosen_id)
            return {
                "current_strategy_id": None,
                "current_strategy": None,
        ...
        logger.info("[StrategyResolver] 选中策略: %s", chosen_id)
        return {
