...
- age：用户说「我xx岁/今年xx/年龄xx/xx岁了」等（阿拉伯数字或中文数字均可）。
  ...
  - evidence：user_input 中表年龄的原文片段，如「二十七岁」或「27岁」，逐字一致。
...
（输出格式由系统约束。）"""
        ...
        if _up or user_src.strip():
            logger.debug(
                "[MemoryManager] basic_info 抽取: LLM 返回 updates=%s confidence=%s evidence=%s user_input_len=%d",
                _up,
                {k: basic_conf.get(k) for k in ("name", "age", "gender", "occupation", "location") if basic_conf.get(k) is not None},
                {k: (str(basic_ev.get(k)) or "")[:40] for k in ("name", "age", "gender", "occupation", "location") if basic_ev.get(k)},
                len(user_src),
            )
        ...
        for k in ("name", "age", "gender", "occupation", "location"):
            ...
            if conf < TH.get(k, 0.9):
                logger.debug("[MemoryManager] basic_info 门控跳过 %s: 置信度 %.2f < %.2f", k, conf, TH.get(k, 0.9))
                continue

            ev = basic_ev.get(k)
            if ev is None:
                logger.debug("[MemoryManager] basic_info 门控跳过 %s: 无 evidence", k)
                continue
            ev_s = str(ev).strip()
            if not ev_s:
                logger.debug("[MemoryManager] basic_info 门控跳过 %s: evidence 为空", k)
                continue
                    ...
                    if num_part.isdigit():
                        if num_part in user_src:
                            ...
                            if cn and cn in user_src:
                                ...
                                logger.debug("[MemoryManager] basic_info 门控跳过 %s: evidence 不在 user_input 中 (ev=%s...)", k, ev_s[:40])
                                continue
                    else:
                        logger.debug("[MemoryManager] basic_info 门控跳过 %s: evidence 不在 user_input 中 (ev=%s...)", k, ev_s[:40])
                        continue
            ...
            if new_val_s not in ev_s:
                if k == "age" and new_val_s.isdigit() and ev_s in user_src:
                    ...
                    if "岁" in ev_s or "年" in ev_s or "多" in ev_s or any(c.isdigit() for c in ev_s):
                        ...
                        logger.debug("[MemoryManager] basic_info 门控跳过 %s: 值 %r 不在 evidence 中", k, new_val_s)
                        continue
                else:
                    logger.debug("[MemoryManager] basic_info 门控跳过 %s: 值 %r 不在 evidence 中", k, new_val_s)
                    continue
        ...
        if db and relationship_id:
            ...
            try:
                ...
                logger.exception("[MemoryManager] DB 写入失败")
        else:
            # local store 模式
            try:
        ...
        if wrote_basic_keys or wrote_profile_keys:
            logger.info(
                "[MemoryManager] wrote_basic=%s wrote_profile=%s",
                wrote_basic_keys or "(none)",
        ...
        if new_topics:
            ...
            logger.info("[MemoryManager] topic_history updated: added %d new topic(s), total=%d", len(new_topics), len(unique_topics))
        else:
