...
def apply_reflection_patch(
    ...
    """
    应用 reflection_patch 到 requirements，修改 plan_goals、style_targets、stage_targets。
    
    Args:
        requirements: 当前的 RequirementsChecklist
        reflection_patch: 结构化 patch，包含 plan_patch、style_patch、stage_patch、search_patch
    
    Returns:
        修改后的 RequirementsChecklist
    """
    if not reflection_patch:
        return requirements
    
    # 创建副本以避免修改原始对象
    patched_requirements = dict(requirements)

    # TTL/衰减：避免 patch 叠加失控（每次应用强度随 ttl_remaining 线性衰减）
    ttl_turns = reflection_patch.get("ttl_turns")
    ttl_remaining = reflection_patch.get("ttl_remaining")
    try:
        ttl_turns_i = int(ttl_turns) if ttl_turns is not None else 0
    except Exception:
        ttl_turns_i = 0
    try:
        ttl_remaining_i = int(ttl_remaining) if ttl_remaining is not None else ttl_turns_i
    except Exception:
        ttl_remaining_i = ttl_turns_i
    strength = 1.0
    if ttl_turns_i > 0:
        strength = max(0.0, min(1.0, float(ttl_remaining_i) / float(ttl_turns_i)))
    
    # ==========================================
    # 1. plan_patch: 补漏 must_cover_points
    # ==========================================
    plan_patch = reflection_patch.get("plan_patch", {})
    if plan_patch and isinstance(plan_patch, dict):
        plan_goals = dict(patched_requirements.get("plan_goals", {}))
        
        # 添加 must_cover_points
        add_points = plan_patch.get("add_must_cover_points", [])
        if isinstance(add_points, list) and add_points:
            existing_points = plan_goals.get("must_cover_points", [])
            if not isinstance(existing_points, list):
                existing_points = []
            # 去重添加
            for point in add_points:
                point_str = str(point).strip()
                if point_str and point_str not in existing_points:
                    existing_points.append(point_str)
            plan_goals["must_cover_points"] = existing_points
        
        # 移除 must_cover_points
        remove_points = plan_patch.get("remove_must_cover_points", [])
        if isinstance(remove_points, list) and remove_points:
            existing_points = plan_goals.get("must_cover_points", [])
            if isinstance(existing_points, list):
                remove_set = {str(x).strip() for x in remove_points}
                plan_goals["must_cover_points"] = [p for p in existing_points if str(p).strip() not in remove_set]
        
        patched_requirements["plan_goals"] = plan_goals

        # 冲突检测：must_cover_points 不得与 avoid_points/forbidden 重叠（最低配：精确去重）
        try:
            avoid_points = plan_goals.get("avoid_points", [])
            avoid_set = {str(x).strip() for x in avoid_points} if isinstance(avoid_points, list) else set()
            forbidden = patched_requirements.get("forbidden", [])
            forb_set = {str(x).strip() for x in forbidden} if isinstance(forbidden, list) else set()
            must_cover = plan_goals.get("must_cover_points", [])
            if isinstance(must_cover, list) and (avoid_set or forb_set):
                filtered = [p for p in must_cover if str(p).strip() and str(p).strip() not in avoid_set and str(p).strip() not in forb_set]
                plan_goals["must_cover_points"] = filtered
                patched_requirements["plan_goals"] = plan_goals
        except Exception:
            pass
    
    # ==========================================
    # 2. style_patch: 调整 style_targets
    # ==========================================
    style_patch = reflection_patch.get("style_patch", {})
    if style_patch and isinstance(style_patch, dict):
        style_targets = dict(patched_requirements.get("style_targets", {}))
        
        for dim, adjustment in style_patch.items():
            if not isinstance(adjustment, (int, float)):
                continue
            
            current_val = style_targets.get(dim, 0.5)  # 默认值 0.5
            ...
            # 正数/0..1：按“绝对目标”处理，但用 strength 做插值，避免一次推过界后长期带偏
            if 0.0 <= float(adjustment) <= 1.0:
                target = float(adjustment)
                new_val = current_val_f + (target - current_val_f) * float(strength)
                style_targets[dim] = max(0.0, min(1.0, float(new_val)))
            else:
                # 其余：按“相对调整(delta)”处理，并随 strength 衰减
                delta = float(adjustment) * float(strength)
                new_val = current_val_f + delta
                style_targets[dim] = max(0.0, min(1.0, float(new_val)))
        
        patched_requirements["style_targets"] = style_targets
    
    # ==========================================
    # 3. stage_patch: 加强 stage_targets.pacing_notes 或提高 violation_sensitivity
    # ==========================================
    stage_patch = reflection_patch.get("stage_patch", {})
    if stage_patch and isinstance(stage_patch, dict):
        stage_targets = dict(patched_requirements.get("stage_targets", {}))
        
        # 添加 pacing_notes
        add_notes = stage_patch.get("add_pacing_notes", [])
        if isinstance(add_notes, list) and add_notes:
            existing_notes = stage_targets.get("pacing_notes", [])
            if not isinstance(existing_notes, list):
                existing_notes = []
            # 去重添加
            for note in add_notes:
                note_str = str(note).strip()
                if note_str and note_str not in existing_notes:
                    existing_notes.append(note_str)
            stage_targets["pacing_notes"] = existing_notes
        
        # 调整 violation_sensitivity
        adjust_sensitivity = stage_patch.get("adjust_violation_sensitivity")
        if adjust_sensitivity is not None:
            try:
                current_sensitivity = float(stage_targets.get("violation_sensitivity", 0.7) or 0.7)
                new_sensitivity = current_sensitivity + float(adjust_sensitivity) * float(strength)
                stage_targets["violation_sensitivity"] = max(0.0, min(1.0, new_sensitivity))
            except (TypeError, ValueError):
                pass
        
        patched_requirements["stage_targets"] = stage_targets
    
    # ==========================================
    # 4. search_patch: 调整 query_seeds（这个需要在 reasoner 的 search_spec 中应用，不在 requirements 中）
    # ==========================================
    # search_patch 需要在 reasoner 的 search_spec 中应用，这里只记录到 requirements 的元数据中
    search_patch = reflection_patch.get("search_patch", {})
    if search_patch:
        # 将 search_patch 存储到 requirements 的元数据中，供后续使用
        if "meta" not in patched_requirements:
            patched_requirements["meta"] = {}
        patched_requirements["meta"]["search_patch"] = search_patch
    
    return patched_requirements  # type: ignore[return-value]
