...
# ═══════════════════════════════════════════════════════════════════════════
RATE_MULTIPLIER = 5.0  # 速率倍率（原20.0÷4）：1.0=原始速率，越大关系/状态变化越快
...
def _get_task_completion_from_state(state: Dict[str, Any]) -> tuple:
    """仅以「已写入 DB」为任务完成：completed 来自 state（上轮 memory_manager 根据 basic_info 写入更新），不读 analyzer/规则。"""
    completed_ids = {str(x) for x in (state.get("completed_task_ids") or []) if str(x).strip()}
    attempted_ids = set()  # 不再使用 attempted 判定，仅保留「写入 DB」为完成
...
def _detect_completed_tasks_and_replenish(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        if completed_ids or added_backlog or trimmed_backlog or bumped:
            print(
    ...
    # completed_task_ids / attempted_task_ids 仅由 memory_manager 根据「写入 DB」写入，此处只更新任务池
    return {
