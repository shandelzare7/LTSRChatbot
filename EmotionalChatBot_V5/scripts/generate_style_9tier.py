"""
测试 9 档裸标签（无 anchor 描述）对 LLM 输出的控制效果。
标签：extremely_low / very_low / low / moderately_low / mid /
...
async def main():
    ...
    for dim in CONTINUOUS_DIMS:
        for tier in TIERS_9:
            ...
            tasks.append(gen(llm, DIM_ZH[dim], TIER_ZH_9[tier], st))

    results = await asyncio.gather(*tasks)

    print("=" * 70)
    print("9 档裸标签测试（无 anchor 描述）")
    ...
    out = os.path.join(ROOT, "scripts", "output", "style_9tier_examples.json")
