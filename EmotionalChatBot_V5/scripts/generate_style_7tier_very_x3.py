"""
7 档裸标签测试（very 版）× 3 次重复。
标签：extremely_low / very_low / low / mid / high / very_high / extremely_high
"""
...
async def main():
    ...
    for dim in CONTINUOUS_DIMS:
        for tier in TIERS_7:
            ...
            for r in range(1, REPEATS + 1):
                tasks.append(gen(llm, DIM_ZH[dim], TIER_ZH_7[tier], st, r))

    results = await asyncio.gather(*tasks)

    print("=" * 70)
    print(f"7 档裸标签测试（very 版）× {REPEATS} 次")
    ...
    out = os.path.join(ROOT, "scripts", "output", "style_7tier_very_x3.json")
