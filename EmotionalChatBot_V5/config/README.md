...
| **content_moves.yaml** | app.lats.reply_planner.plan_reply_27_via_content_moves, utils.yaml_loader.load_content_moves | LATS 27 候选：需至少 8 条 move，每条含 `tag`（及可选 zh、brief）；前 8 路按 tag 各生成 3 条，第 9 路为 FREE。应纳入版本控制并与 reply_planner 约定保持一致。 |
