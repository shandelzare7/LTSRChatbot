...
- knowledge_gap：以下任一即填 true：①用户**问事实、问数据、问现状、问发生了什么**（谁/什么/多少/何时/哪里/为什么等）；②用户**提到了专有名词、生僻词、你不确定的概念或需要查证才能准确回应**的内容。**仅当**用户明显只是在表达情绪、感受、观点、打招呼或纯闲聊（且未涉及上述可查证/未知词）时填 false。拿不准时倾向填 true。
- search_keywords：knowledge_gap=true 时从用户问题里提炼 3–8 字搜索关键词（中文），可包含专有名词或核心概念；否则填空字符串。
